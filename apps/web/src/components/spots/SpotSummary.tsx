...
import SpotAmenities from "@/components/spots/SpotAmenities";
import SpotPolicies from "@/components/spots/SpotPolicies";
import SpotTags from "@/components/spots/SpotTags";
...
function OpeningHoursBlock({ hours }: { hours?: OpeningHours }) {
  ...
  const rows = Array.isArray(hours)
    ...
    : (["mon", "tue", "wed", "thu", "fri", "sat", "sun"] as const).map((d) => {
        ...
        const nameMap: Record<string, string> = { mon: "월", tue: "화", wed: "수", thu: "목", fri: "금", sat: "토", sun: "일" };
...
export default function SpotSummary({ spot, className = "", onSuggestInfo }: Props) {
  ...
  const amenities: string[] = useMemo(
    () => (((spot as any)?.amenities ?? []) as string[]).filter(Boolean),
    [spot]
  );
  ...
  const isEmptyPolicies = Object.values(policies).every((v) => !v);
