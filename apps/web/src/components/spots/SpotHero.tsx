...
import SpotActionBar from "@/components/spots/detail/SpotActionBar";
...
/**
 * 히어로는 LCP 후보다. eager + fetchpriority="high" 권장. sizes 힌트 제공. :contentReference[oaicite:3]{index=3}
 * width/height 지정으로 초기 레이아웃 면적을 고정해 CLS 방지. :contentReference[oaicite:4]{index=4}
 */
export default function SpotHero({ spot, isFavorited, onFavoriteToggle, galleryImages = [] }: Props) {
  const navigate = useNavigate();

  const cover = useMemo(() => {
    const first = (spot as any)?.heroImage || (spot as any)?.imageUrl || (spot as any)?.imageUrls?.[0];
    return first || galleryImages[0] || PLACEHOLDER;
  }, [spot, galleryImages]);
      ...
      <div className="relative h-[320px] w-full overflow-hidden md:h-[420px] lg:h-[460px]">
        ...
        <div className="absolute left-0 right-0 top-0 mx-auto flex max-w-screen-xl items-center justify-between px-4 py-4 md:px-10 lg:px-20">
          ...
          <div className="flex items-center gap-2">
            <button onClick={share} aria-label="공유" className="rounded-full bg-black/40 p-2 text-white backdrop-blur hover:bg-black/60">
              <FiShare2 />
            </button>
            <button
              onClick={onFavoriteToggle}
              aria-label={isFavorited ? "관심 해제" : "관심 등록"}
              className={`rounded-full p-2 backdrop-blur ${isFavorited ? "bg-primary text-white" : "bg-black/40 text-white hover:bg-black/60"}`}
            >
              <FiHeart />
            </button>
          </div>
        ...
        <div className="absolute inset-x-0 bottom-0 mx-auto max-w-screen-xl px-4 pb-6 md:px-10 lg:px-20">
          <div className="max-w-3xl">
            ...
            {(spot as any)?.address && <p className="mt-2 text-sm text-white/80">{(spot as any).address}</p>}
          </div>
        </div>
      </div>
