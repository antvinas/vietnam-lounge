...
const ExchangeTicker = () => {
  ...
  if (isLoading || isError || !data) {
    return <div className="h-6 w-full animate-pulse rounded-full bg-black/10" />;
  }

  const { rates } = data;
  const vndRate = rates.VND.toLocaleString("en-US", { maximumFractionDigits: 0 });
  const krwRate = rates.KRW.toLocaleString("en-US", { maximumFractionDigits: 2 });
