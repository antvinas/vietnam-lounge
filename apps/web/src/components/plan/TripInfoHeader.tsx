// apps/web/src/components/plan/TripInfoHeader.tsx
import React, { useMemo, useState } from "react";
import { usePlanStore, selectDaysOfTrip } from "@/store/plan.store";
import { usePlanUIStore } from "@/store/plan.ui.store";
import TripInfoEditor from "@/components/plan/TripInfoEditor";
...
export default function TripInfoHeader() {
  ...
  const dayContextLabel = useMemo(() => {
    ...
    const idx = (daysOfTrip as any[]).findIndex(
      ...
      <section className="flex w-full flex-col gap-1 pb-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="shrink-0 rounded-md border border-gray-300 bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 dark:hover:bg-gray-800"
          >
            ✏️ 여행 정보 수정
          </button>
        </div>
