...
// Re-using components from SpotsHome - in a real-world scenario, these would be in a shared components folder.
...
const Region = () => {
  ...
  const filteredAndSortedSpots = useMemo(() => {
    if (!spots) return [];
    return spots
      .filter(spot => spot.region === regionName)
  ...
  }, [spots, regionName, filterCategory, sortBy]);

  const categories = useMemo(() => {
    if (!spots) return ['All'];
    const regionSpots = spots.filter(s => s.region === regionName);
    return ['All', ...new Set(regionSpots.map(s => s.category))];
  }, [spots, regionName]);

  if (isError) return <div className="text-center py-10 text-red-500">Failed to load spots for this region.</div>;

  return (
    <main className="container mx-auto px-4 py-12">
      <div className="text-center mb-12">
        <h1 className="text-5xl font-extrabold text-gray-900 dark:text-white">Explore {regionName}</h1>
        <p className="text-xl text-gray-600 dark:text-gray-400 mt-4">Discover the best spots in the {regionName} region.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-10">
            <p className="text-xl text-gray-500">No spots found in {regionName} with the selected filters.</p>
        </div>
      )}
    </main>
  );
