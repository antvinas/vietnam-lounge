...
import { getBookmarkedPosts } from '../../api/user.api';
import { Link } from 'react-router-dom';
import { FaEye, FaThumbsUp, FaComment } from 'react-icons/fa';

const MyFavorites = () => {
  const { data: posts, isLoading, isError } = useQuery({ queryKey: ['bookmarkedPosts'], queryFn: getBookmarkedPosts });

  if (isLoading) return <div>Loading your favorite posts...</div>;
  if (isError) return <div>Failed to load favorite posts.</div>;
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md">
      ...
      {posts && posts.length > 0 ? (
        ...
        <p className="p-6 text-center text-gray-500">You haven't bookmarked any posts yet.</p>
      )}
    </div>
  );
};
