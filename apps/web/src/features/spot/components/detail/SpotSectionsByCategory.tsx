...
export default function SpotSectionsByCategory({ spot, className = "" }: Props) {
  ...
  const isRestaurant = /(restaurant|food|cafe|카페|레스토랑|식당|음식)/.test(c);
  const isBar = /(bar|pub|nightclub|바|펍)/.test(c);
  const isHotel = /(hotel|숙소|호텔)/.test(c);

  return (
    <section className={`space-y-6 ${className}`}>
      {(isRestaurant || isBar) && (
        <article className="rounded-2xl border border-slate-700 bg-slate-800/40 p-4">
          <h3 className="mb-2 text-base font-semibold text-slate-100">메뉴 · 가격대</h3>
          <p className="text-sm text-slate-200">
            {(spot as any)?.priceRange || "가격대 정보 없음"}
          </p>
          {(spot as any)?.menuUrl ? (
            <a
      ...
      {isHotel && (
        <article className="rounded-2xl border border-slate-700 bg-slate-800/40 p-4">
          <h3 className="mb-2 text-base font-semibold text-slate-100">체크인 · 체크아웃 · 정책</h3>
          <p className="text-sm text-slate-200">
            {(spot as any)?.checkInOut || "체크인/체크아웃 정보 없음"}
          </p>
          <p className="mt-2 text-sm text-slate-300">
            {(spot as any)?.cancellationPolicy || "취소/환불 정책 미등록"}
          </p>
