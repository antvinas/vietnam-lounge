...
const getCollectionName = (
  segment: string,
  type: "posts" | "comments" | "categories"
) => {
...
router.get("/posts/:segment", async (req, res) => {
  ...
  const { category, region, sort = "latest", cursor = null, limit = 10 } =
    req.query;

  const collectionName = getCollectionName(segment, "posts");

  try {
    let query: FirebaseFirestore.Query = getDb().collection(collectionName);
    ...
    if (sort === "latest") query = query.orderBy("createdAt", "desc");
    else if (sort === "hot") query = query.orderBy("likes", "desc");
    else if (sort === "comments")
      query = query.orderBy("commentsCount", "desc");
    else if (sort === "views") query = query.orderBy("viewCount", "desc");

    if (cursor) {
      const cursorDoc = await getDb().collection(collectionName).doc(cursor as string).get();
...
router.post("/posts", requireAuth, async (req, res) => {
  const { title, content, category, segment, region, isNotice, isPinned } =
    req.body;
