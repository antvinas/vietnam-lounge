...
router.get('/detail', async (req, res) => {
    ...
    try {
        const doc = await getDb().collection('spots').doc(id).get();
        if (!doc.exists)
...
router.get('/recommendations', async (req, res) => {
    ...
    try {
        const spotDoc = await getDb().collection('spots').doc(spotId).get();
        if (!spotDoc.exists || !spotDoc.data())
...
router.get('/reviews', async (req, res) => {
    ...
    if (!spotId)
        return res.status(400).send({ error: 'Spot ID is required.' });
    try {
        const snapshot = await getDb().collection('spots').doc(spotId).collection('reviews').orderBy('createdAt', 'desc').get();
        res.status(200).send(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    }
    catch (error) {
        console.error(error);
