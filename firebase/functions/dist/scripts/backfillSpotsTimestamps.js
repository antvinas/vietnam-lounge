...
function parseArgs(argv) {
    ...
    const mode = args.has("--commit") ? "commit" : "dry";
    const pageSizeRaw = (_a = argv.find((x) => x.startsWith("--pageSize="))) === null || _a === void 0 ? void 0 : _a.split("=")[1];
    const pageSize = clampInt(pageSizeRaw, 50, 500, 500);
    const onlyRaw = (_b = argv.find((x) => x.startsWith("--only="))) === null || _b === void 0 ? void 0 : _b.split("=")[1];
    const only = onlyRaw
        ? onlyRaw
            .split(",")
            .map((s) => s.trim())
...
function asTimestamp(v, fallback) {
    try {
...
function metaTimeOrNow(snap) {
    var _a, _b;
    // createTime/updateTime는 admin SDK에서 Timestamp로 제공됨
    return (_b = (_a = snap.createTime) !== null && _a !== void 0 ? _a : snap.updateTime) !== null && _b !== void 0 ? _b : admin.firestore.Timestamp.now();
}
async function backfillCollection(db, collectionName, pageSize, mode, verbose) {
    var _a;
    let scanned = 0;
    let updated = 0;
    let skipped = 0;
    ...
    while (true) {
        ...
        for (const docSnap of snap.docs) {
            ...
            const updatedAt = hasUpdated ? tsOrNow(data.updatedAt) : ((_a = docSnap.updateTime) !== null && _a !== void 0 ? _a : createdAt);
            const patch = {};
            if (!hasCreated)
            ...
            if (verbose) {
                ...
                console.log(`[${collectionName}] patch ${docSnap.id}:`, Object.keys(patch).join(", "));
            }
            if (mode === "commit") {
        ...
        // 안전 로그
        // eslint-disable-next-line no-console
        console.log(`[${collectionName}] progress: scanned=${scanned}, updated=${updated}, skipped=${skipped}, lastDocId=${lastDocId}`);
    }
...
async function main() {
    ...
    for (const r of results) {
        // eslint-disable-next-line no-console
        console.log(`${r.collection}: scanned=${r.scanned}, updated=${r.updated}, skipped=${r.skipped}, batches=${r.batches}`);
    }
    // eslint-disable-next-line no-console
    console.log(`TOTAL: scanned=${total.scanned}, updated=${total.updated}, skipped=${total.skipped}, batches=${total.batches}`);
    // eslint-disable-next-line no-console
    console.log("================================\n");
    if (mode !== "commit") {
        // eslint-disable-next-line no-console
        console.log("DRY RUN 완료(쓰기 없음). 실제 반영하려면: --commit 옵션을 붙여서 실행하세요.");
    }
}
main().catch((e) => {
    // eslint-disable-next-line no-console
...
//# sourceMappingURL=backfillSpotsTimestamps.js.map